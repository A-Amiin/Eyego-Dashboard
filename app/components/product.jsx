    ...
    return (
        <div className="p-4 bg-white rounded-lg shadow-sm w-full overflow-x-auto">
            ...
            {searchQuery && filteredData.length === 0 ? (
                <p className="text-center py-8 text-red-500">No product found with the name "{searchQuery}"</p>
            ) : (
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between text-xs md:text-sm text-gray-500">
                <p>Showing {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, filteredData.length)} of {filteredData.length} products</p>
                <div className="flex items-center space-x-2">
                    ...
                    <button onClick={nextPage} className="cursor-pointer px-2 py-1 border rounded hover:bg-gray-50" disabled={currentPage === Math.ceil(filteredData.length / itemsPerPage)}>Next</button>
                </div>
            </div>
        </div>
    );
